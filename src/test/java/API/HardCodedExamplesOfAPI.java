...
public class HardCodedExamplesOfAPI {
    ...
    String token = "Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpYXQiOjE2MzkyNDUxNzYsImlzcyI6ImxvY2FsaG9zdCIsImV4cCI6MTYzOTI4ODM3NiwidXNlcklkIjoiMzIzNSJ9.3PIQmxkSM6-jdNIm1Oo2BifjWUELvBfuOODO9OX-gKk";
    ...
    public void getDetailsOfOneEmployee() {
        ...
        //given
        RequestSpecification preparedRequest = given().header("Authorization", "").header("Content-Type", "application/json").queryParam("employee_id", "25598A");
    ...
    public void createEmployee() {
        ...
        RequestSpecification preparedRequest = given().header("Authorization", token).header("Content-Type", "application/json").body("{\n" +
                "  \"emp_firstname\": \"Olzhars\",\n" +
                "  \"emp_lastname\": \"Andru\",\n" +
                "  \"emp_middle_name\": \"elenam\",\n" +
                "  \"emp_gender\": \"F\",\n" +
        ...
        //when
        Response response = preparedRequest.when().get("/createEmployee.php");
        response.prettyPrint();
        //this pretty print does the same job as sout.
        ...
        response.then().assertThat().body("Employee.emp_firstname", equalTo("Olzhars"));
    ...
    public void getCreatedEmployee() {
        ...
        Assert.assertTrue(firstName.contentEquals("Olzhars"));

        String lastName = response.jsonPath().getString("Employee.emp_lastname");
        Assert.assertTrue(lastName.contentEquals("Andru"));
    ...
    public void updatedCreatedEmployee() {
        RequestSpecification preparedRequest = given().header("Authorization", token).
                header("Content-Type", "application/json").body("{\n" +
                        ...
                        "  \"emp_firstname\": \"Jose\",\n" +
                        "  \"emp_lastname\": \"EGO1\",\n" +
                        "  \"emp_middle_name\": \"Antonio\",\n" +
                        "  \"emp_gender\": \"M\",\n" +
                        "  \"emp_birthday\": \"2001-01-12\",\n" +
                        "  \"emp_status\": \"Active\",\n" +
                        "  \"emp_job_title\": \"Tester\"\n" +
                        "}");


        Response response= preparedRequest.when().put("/updatedEmployee.php");
        response.prettyPrint();
